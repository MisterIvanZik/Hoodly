...
export default function PlanningPage() {
  ...
  const [selectedEvent, setSelectedEvent] = useState<Conversation | null>(null)
  ...
  const getEventsForDay = (day: number) => {
    return confirmedBookings.filter((booking) => {
      const bDate = new Date(booking.creneau!.date)
      return (
        bDate.getDate() === day &&
  ...
  const handleFinish = async (conv: Conversation) => {
    ...
    try {
      ...
      toast.success('Prestation marquée comme accomplie avec succès !')
      setSelectedEvent(null)
    } catch {
  ...
  const handleValidate = async (conv: Conversation) => {
    ...
    try {
      ...
      toast.success('Félicitations, réalisation validée et transaction réglée !')
      setSelectedEvent(null)
    } catch (err: any) {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto pb-24 space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1 text-sm font-light leading-relaxed">
            Consultez toutes vos prestations d'entraide validées et gérez vos rendez-vous de bon voisinage.
          </p>
        </div>
        ...
        <Card className="lg:col-span-2 bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex gap-1 bg-gray-50 rounded-xl p-1 border">
              <button
                ...
                className="p-2 hover:bg-white hover:text-gray-900 rounded-lg text-gray-400 transition-all shadow-xs"
              >
          ...
          <div className="grid grid-cols-7 gap-2">
            {['Lun', 'Mar', 'Mer', 'Jeu', 'Ven', 'Sam', 'Dim'].map((d) => (
              <div key={d} className="text-center py-2 text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                {d}
              </div>
            ))}
            ...
            {daysArray.map((day, idx) => {
              if (day === null) {
                return <div key={`empty-${idx}`} className="aspect-square bg-gray-50/30 rounded-2xl border border-dashed border-gray-100" />
              }
              ...
              return (
                <div
                  key={`day-${day}`}
                  className={`aspect-square rounded-2xl border p-2 flex flex-col justify-between transition-all group overflow-hidden ${
                    isToday
                      ? 'border-[#2c308e] bg-indigo-50/20'
                      : 'border-gray-100 hover:border-gray-200 bg-white'
                  }`}
                >
                  <span className={`text-xs font-bold ${
                    isToday ? 'text-[#2c308e]' : 'text-gray-400'
                  }`}>
                    {day}
                  </span>
                  ...
                  <div className="space-y-1 overflow-hidden mt-1 flex-1 flex flex-col justify-end">
                    {dayEvents.slice(0, 2).map((ev) => {
                      const style = ev.serviceId ? (CATEGORY_STYLES[ev.serviceId.categorie] || DEFAULT_STYLE) : DEFAULT_STYLE
                      return (
                        <button
                          key={ev._id}
                        ...
                        >
                          {ev.creneau?.debut} {ev.serviceId?.titre}
                        </button>
                      )
                    })}
                    {dayEvents.length > 2 && (
                      <span className="text-[7px] text-gray-400 text-center font-bold">
          ...
          <div className="space-y-4">
            {upcomingBookings.length === 0 ? (
              <div className="p-8 bg-white border border-dashed rounded-[2rem] text-center text-gray-400">
                ...
                <p className="text-xs font-bold text-gray-900">Aucun cours ou service planifié</p>
                <p className="text-[10px] mt-1 leading-relaxed">
                  Proposez des créneaux dans vos discussions privées avec vos voisins pour réserver des prestations.
                </p>
              </div>
            ) : (
              upcomingBookings.map((booking) => {
                const other = getOtherParticipant(booking)
                const dateStr = new Date(booking.creneau!.date).toLocaleDateString('fr-FR', {
                  day: 'numeric',
                  month: 'short'
                })
                ...
                return (
                  ...
                  >
                    <div className="h-12 w-12 rounded-xl bg-gray-50 flex flex-col items-center justify-center border shrink-0">
                      <span className="text-xs font-extrabold text-[#2c308e] leading-none uppercase">
                        {dateStr.split(' ')[1]}
                      </span>
                      <span className="text-lg font-extrabold text-gray-900 leading-none mt-0.5">
                        {dateStr.split(' ')[0]}
                      </span>
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span className={`text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded border ${style.bg} ${style.text}`}>
                          {booking.serviceId?.categorie}
                        </span>
                        <span className="text-[8px] text-gray-400 font-bold uppercase tracking-wider">
                          {booking.creneau?.debut} - {booking.creneau?.fin}
                        </span>
                      </div>
                      <h4 className="text-xs font-bold text-gray-900 truncate mt-1">
                        {booking.serviceId?.titre}
                      </h4>
                      <p className="text-[9px] text-gray-400 truncate font-light">
                        Voisin : {other?.name || 'Voisin'}
                      </p>
                    </div>
                  </Card>
                )
      ...
      {selectedEvent && selectedEvent.serviceId && selectedEvent.creneau && (() => {
        const ev = selectedEvent
        const service = ev.serviceId!
        const creneau = ev.creneau!
        const other = getOtherParticipant(ev)
        const dateStr = new Date(creneau.date).toLocaleDateString('fr-FR', {
          weekday: 'long',
          year: 'numeric',
          month: 'long',
          day: 'numeric'
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-[2rem] border border-gray-100 overflow-hidden shadow-2xl max-w-md w-full flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-200 relative">
              <button
                ...
                className="absolute top-4 right-4 bg-gray-50 hover:bg-gray-100 text-gray-500 rounded-full p-2 transition-colors z-10"
              >
                <X className="h-4 w-4" />
              </button>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2 pt-2">
                    ...
                    <div className="flex items-center gap-2 text-xs text-gray-500 font-light">
                      ...
                      <span>Horaires : de <strong>{creneau.debut}</strong> à <strong>{creneau.fin}</strong></span>
                    </div>
                  </div>
                </div>
                ...
                <div className="border-t border-gray-100 pt-6 space-y-3">
                  {prestationStatut === 'valide' && isProvider && (
                    <Button
                      ...
                      className="w-full bg-[#2c308e] hover:bg-[#2c308e]/95 text-white rounded-xl py-5 text-xs font-bold shadow-md flex items-center justify-center gap-2"
                    >
                  ...
                  {prestationStatut === 'en_cours' && isProvider && (
                    <Button
                      onClick={() => handleFinish(ev)}
                      className="w-full bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl py-5 text-xs font-bold shadow-md flex items-center justify-center gap-2"
                    >
                      <CheckCircle2 className="h-4 w-4" />
                      <span>Marquer comme accompli</span>
                    </Button>
                  )}
                  ...
                  {prestationStatut === 'termine' && !realisationValidee && isClient && (
                    ...
                    >
                      ...
                      <span>Valider la réalisation (Payer)</span>
                    </Button>
                  )}
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      onClick={() => {
                        setSelectedEvent(null)
                        navigate(`/messages?id=${ev._id}`)
                      }}
                      className="flex-1 rounded-xl py-5 text-xs font-semibold flex items-center justify-center gap-2"
                    >
                      <MessageSquare className="h-4 w-4 text-gray-500" />
                      <span>Écrire au voisin</span>
                    </Button>
